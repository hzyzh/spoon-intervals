 ...
 */
public class IControlFlowBuilder implements CtVisitor {

	IControlFlowGraph result = new IControlFlowGraph(IControlFlowEdge.class);

	IControlFlowNode exitNode = new IControlFlowNode(null, result, BranchKind.EXIT);

	IControlFlowNode beginNode = new IControlFlowNode(null, result, BranchKind.BEGIN);

	IControlFlowNode lastNode = beginNode;

	HashMap<String, CtStatement> labeledStatement = new HashMap<>();
	List<CtMethod> calleesPool;

	//This stack pushes all the nodes to wich a break statement may jump to.
	Stack<IControlFlowNode> breakingBad = new Stack<>();
	//This stack pushes all the nodes to wich a continue statement may jump to.
	Stack<IControlFlowNode> continueBad = new Stack<>();

	public IControlFlowBuilder(List<CtMethod> pool) {
		super();
		if(pool != null)
			calleesPool = pool;
		else
	...
	public IControlFlowBuilder() {
		super();
		calleesPool = new ArrayList<>();
	}

	public IControlFlowGraph getResult() {
		return result;
	}
	 ...
	 */
	public IControlFlowGraph build(CtElement s) {
		s.accept(this);
	...
	private void visitConditional(CtElement parent, CtConditional conditional) {
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(conditional.getCondition());
		IControlFlowNode branch = new IControlFlowNode(parent, result, BranchKind.BRANCH, visitor.getCallees());
		tryAddEdge(lastNode, branch);

		IControlFlowNode convergenceNode = new IControlFlowNode(null, result, BranchKind.CONVERGE);
		lastNode = branch;
		if (conditional.getThenExpression() instanceof CtConditional) {
			...
			calleeVisitor visitor2 = new calleeVisitor(calleesPool);
			visitor2.scan(conditional.getThenExpression());
			lastNode = new IControlFlowNode(conditional.getThenExpression(), result, BranchKind.STATEMENT, visitor2.getCallees());
		...
		if (conditional.getElseExpression() instanceof CtConditional) {
			...
			calleeVisitor visitor3 = new calleeVisitor(calleesPool);
			visitor3.scan(conditional.getElseExpression());
			lastNode = new IControlFlowNode(conditional.getElseExpression(), result, BranchKind.STATEMENT, visitor3.getCallees());
			tryAddEdge(branch, lastNode);
		}
		tryAddEdge(lastNode, convergenceNode);
	 ...
	 */
	public static IControlFlowNode firstNode(IControlFlowGraph g, CtElement statement) throws NotFoundException {
		...
		if (statement instanceof CtFor) {
			...
			IControlFlowNode n = g.findNode(((CtDo) statement).getLoopingExpression());
			IControlFlowNode n1 = null;
			for (IControlFlowEdge e : g.outgoingEdgesOf(n)) {
				if (e.isBackEdge()) {
					n1 = e.getTargetNode();
					break;
				}
	...
	private void defaultAction(BranchKind kind, CtStatement st) {
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(st);
		IControlFlowNode n = new IControlFlowNode(st, result, kind, visitor.getCallees());
	 ...
	 */
	private void tryAddEdge(IControlFlowNode source, IControlFlowNode target) {
	 ...
	 */
	private void tryAddEdge(IControlFlowNode source, IControlFlowNode target, boolean isLooping, boolean breakDance) {
		...
		if (source != null && target != null
			...
			IControlFlowEdge e = result.addEdge(source, target);
	...
	private <R> void travelStatementList(List<CtStatement> statements) {
		IControlFlowNode begin = new IControlFlowNode(null, result, BranchKind.BLOCK_BEGIN);
		...
		IControlFlowNode end = new IControlFlowNode(null, result, BranchKind.BLOCK_END);
	...
	public void visitCtDo(CtDo doLoop) {
		...
		IControlFlowNode convergenceNodeOut = new IControlFlowNode(null, result, BranchKind.CONVERGE);
		breakingBad.push(convergenceNodeOut);


		tryAddEdge(lastNode, convergenceNode);
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(doLoop.getLoopingExpression());
		IControlFlowNode branch = new IControlFlowNode(doLoop.getLoopingExpression(), result, BranchKind.BRANCH, visitor.getCallees());
	...
	public void visitCtFor(CtFor forLoop) {
		...
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(forLoop.getExpression());
		IControlFlowNode convergence = new IControlFlowNode(forLoop.getExpression(), result, BranchKind.CONVERGE, visitor.getCallees());
		breakingBad.push(convergence);

		//Next the branch
		IControlFlowNode branch = new IControlFlowNode(forLoop.getExpression(), result, BranchKind.BRANCH, visitor.getCallees());
		tryAddEdge(lastNode, branch);
	...
	public void visitCtForEach(CtForEach foreach) {
		...
		IControlFlowNode convergence = new IControlFlowNode(null, result, BranchKind.CONVERGE);
		breakingBad.push(convergence);

		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(foreach.getVariable());
		IControlFlowNode init = new IControlFlowNode(foreach.getVariable(), result, BranchKind.STATEMENT, visitor.getCallees());
		tryAddEdge(lastNode, init);
		lastNode = init;

		calleeVisitor visitor2 = new calleeVisitor(calleesPool);
		visitor2.scan(foreach.getExpression());
		IControlFlowNode branch = new IControlFlowNode(foreach.getExpression(), result, BranchKind.BRANCH, visitor2.getCallees());
		continueBad.push(branch);
		tryAddEdge(lastNode, branch);
	...
	public void visitCtIf(CtIf ifElement) {
		...
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(ifElement.getCondition());
		IControlFlowNode branch = new IControlFlowNode(ifElement.getCondition(), result, BranchKind.BRANCH, visitor.getCallees());
		tryAddEdge(lastNode, branch);
	...
	public <R> void visitCtReturn(CtReturn<R> returnStatement) {
		...
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(returnStatement);
		IControlFlowNode n = new IControlFlowNode(returnStatement, result, BranchKind.STATEMENT, visitor.getCallees());
		tryAddEdge(lastNode, n);
	...
	public <S> void visitCtCase(CtCase<S> caseStatement) {
		...
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(caseStatement.getCaseExpression());
		IControlFlowNode caseNode = new IControlFlowNode(caseStatement.getCaseExpression(), result, BranchKind.STATEMENT, visitor.getCallees());
	...
	public <S> void visitCtSwitch(CtSwitch<S> switchStatement) {
		...
		//Push the condition
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(switchStatement.getSelector());
		IControlFlowNode switchNode = new IControlFlowNode(switchStatement.getSelector(), result, BranchKind.BRANCH, visitor.getCallees());
		...
		for (CtCase caseStatement : switchStatement.getCases()) {
			...
			registerStatementLabel(caseStatement);
			calleeVisitor visitor2 = new calleeVisitor(calleesPool);
			visitor2.scan(caseStatement.getCaseExpression());
			IControlFlowNode cn = new IControlFlowNode(caseStatement.getCaseExpression(), result, BranchKind.STATEMENT, visitor2.getCallees());
	...
	public void visitCtWhile(CtWhile whileLoop) {
		...
		calleeVisitor visitor = new calleeVisitor(calleesPool);
		visitor.scan(whileLoop.getLoopingExpression());
		IControlFlowNode branch = new IControlFlowNode(whileLoop.getLoopingExpression(), result, BranchKind.BRANCH, visitor.getCallees());
		continueBad.push(branch);
